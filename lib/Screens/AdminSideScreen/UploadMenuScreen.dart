...
class _UploadMenuScreenState extends State<UploadMenuScreen> {
  ...
  Future<void> uploadMenuImage()async {
    ...
    if(user !=null){
      try{
        if(_imageFile!=null){
          Reference storageReference =FirebaseStorage.instance.ref().child("MenuImages/${Timestamp.now()}");
          ...
          MenuModel menuModel=MenuModel(
              image: imageUrl,
              dateTime: DateTime.now().day.toString()+"-"+month+"-"+DateTime.now().year.toString());

          await _firestore.collection("admin").doc(uid).collection("menuImage").add({
            "menuPost": menuModel.toMap()
          }).then((value) {
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Menu Uploaded Successfull")));
                      ...
                      Row(
                        children: [
                          Container(
                            ...
                            child: Icon(Icons.videocam, size: 20.sp,color: AppTheme.primaryColor,),
                          ),
                          ...
                          Container(
                            ...
                            child: Icon(Icons.image, size: 20.sp,color: AppTheme.primaryColor,),
                          )
                        ],
                      )
